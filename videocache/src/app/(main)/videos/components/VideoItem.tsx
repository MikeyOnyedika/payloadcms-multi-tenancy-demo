...
export default function VideoItem({
  ...
  return (
    <li className="flex flex-col rounded-md shadow-md h-[15rem] bg-gray-200">
      <Link href={`/videos/${video.name}`} className="flex flex-col relative h-full">
        <input
          type="checkbox"
        ...
        <div className="flex justify-center items-center w-full h-full ">
          <IconContext.Provider
            value={{ className: "text-[4rem] text-gray-500" }}
          >
            <Play />
          </IconContext.Provider>
        </div>
      ...
      <div className="bg-blue-50 p-4 rounded-b-md">
        <h3 className="line-clamp-2 text-blue-500">{video.name}</h3>
      </div>
    </li>
  );
}
